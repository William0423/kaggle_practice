...
def pre_processing(train):
    ...
    sns.heatmap(corrmat, square=True, annot=True, fmt='.2f', annot_kws={
                'size': 15}, xticklabels=features_mean, yticklabels=features_mean, cmap='coolwarm')
...
def bagging_train(X_train, X_test, y_train, y_test):
    ...
    print (bag_clf.__class__.__name__, accuracy_score(
...
def rand_train(X_train, X_test, y_train, y_test):
    print "=============rand_train =============="
    randoem_clf = RandomForestClassifier(
        n_estimators=100)  # a simple random forest model
...
def plot_train_data(train, features_mean):
    ...
    colors = train["diagnosis"].map(lambda x: color_function.get(x))
    pd.scatter_matrix(train[features_mean], c=colors,
                      alpha=0.5, figsize=(15, 15))  # 将df各列分别组合绘制散点图
    plt.xticks(rotation='90')
    plt.yticks(rotation=0)
    plt.show()
...
Here Mean means the means of the all cells, standard Error of all cell and worst means the worst cell
    ...
    prediction_var = ['perimeter_mean', 'texture_mean',
    ...
    # train_test_split()这个方法中参数的格式可以是numpy.ndarray和dataframe
